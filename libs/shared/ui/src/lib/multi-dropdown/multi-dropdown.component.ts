...
export class MultiDropdownComponent extends DestroyableComponent implements OnInit, OnDestroy {
    ...
    this.pathSub = this.searchField$.valueChanges
      .pipe(
        ...
        switchMap((searchValue: any) => {
          ...
          if (this._customFilter) {
            this.queryParams = this.parseFilterParams({ name: '', ...this._customFilter }, this.queryParams, []);
          } else {
      ...
      .subscribe(res => {
        if (res.data?.length) {
          ...
          this.allIds.emit(this.dataSource.map(item => item[this.valueKey]));
  ...
  private setQueryParams(params: ParamMap): void {
    ...
    if (this.path.includes(this.env.api.poses) && params.getAll('posId').length) {
      selectedList = params.getAll('posId').filter(item => item?.toString().trim());
    } else if (this.path.includes(this.env.api.merchants) && params.getAll('merchantId').length) {
