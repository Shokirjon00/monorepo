...
import { ClickOutsideModule, DateFormatEnum, DateTimePipe, DestroyableComponent, ICaption, IOptionAction, IPaginate, IRowAction, ResizeColumnModule, TableCellTooltipDirective, TableFieldTypes, TooltipDirective, TableRowActionEnum, TableStatusEnum, ToastEnum } from '@eskhata/util';
import { finalize, take, takeUntil } from 'rxjs';
import { HttpClient, HttpResponse } from '@angular/common/http';
import { HeaderService, HelperService } from '@eskhata/data-access';
...
import { TABLE_CONFIG } from './table-config.token';
import { EskhataBankLoaderComponent } from '../eskhata-bank-loader/eskhata-bank-loader.component';
...
export class TableComponent extends DestroyableComponent implements AfterViewInit {
  ...
  @Output() changeShift = this.register(new EventEmitter());
  ...
  protected readonly tableConfig = inject(TABLE_CONFIG);
  ...
  downloadFile(fileStorageUrl: string, fileId: string, token: string): void {
    if (this.tableConfig.download === 'save') {
      this.saveFile(fileStorageUrl, fileId, token);
      return;
    }
  ...
  private saveFile(fileStorageUrl: string, fileId: string, token: string): void {
    this.loading = true;
    this.helperService
      .getFile(fileStorageUrl, fileId, token)
      .pipe(
      ...
      .subscribe((res: HttpResponse<Blob>) => {
        ...
        this.fileSaverService.save(res.body, match && match[1] ? match[1].replace(/['"]/g, '') : 'download');
      });
  }
