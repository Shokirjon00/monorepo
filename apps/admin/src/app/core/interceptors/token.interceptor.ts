...
export class TokenInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    const isWebUrl = request.url.includes(env.api.web);
    const isSendWebhook = request.url.includes(env.api.sendWebhook);
    const skipAuth = !request.url.includes(env.api.refresh) && !(isWebUrl && !isSendWebhook);
