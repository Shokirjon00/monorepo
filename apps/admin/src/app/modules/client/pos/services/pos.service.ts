...
export class PosService {
  ...
  getPosConnectionQr(posId: string): Observable<IHttpResponse<string>> {
    return this.http.get<IHttpResponse<string>>(`${this.apiUrl}/${env.api.qr}/${env.api.attach}/${posId}`);
  }
  ...
  getQRCode(id: string): Observable<IHttpResponse<string>> {
    return this.http.get<IHttpResponse<string>>(`${this.apiUrl}/${env.api.qr}/${id}`);
  }
  ...
  getPosDictionaryWithoutPagination(queryParams: Params,  bodyParams: Params): Observable<IHttpResponse<ISelect[]>> {
    const params = new HttpParams({fromObject: queryParams});
    return this.http.post<IHttpResponse<ISelect[]>>(`${this.apiUrl}/${env.api.dictionaryWithoutPagination}`, bodyParams, { params: params } );
  }
