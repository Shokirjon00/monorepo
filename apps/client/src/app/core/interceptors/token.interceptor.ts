...
export class TokenInterceptor implements HttpInterceptor {
  ...
  private getToken(url: string): string {
    ...
    if (url.includes(env.api.setToken)) {
      return this.tokenService.accessToken;
    }

    if (
      url.includes(env.api.authenticate) ||
      ((url.includes(`/${env.api.setPassword}`) || url.includes(`/${env.api.identities}`)) &&
        !url.includes(env.api.logout))
    ) {
