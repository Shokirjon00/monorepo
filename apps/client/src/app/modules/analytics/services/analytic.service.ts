...
export class AnalyticService {
  ...
  getPaymentStatistics(queryParams: Params): Observable<IHttpResponse<IPaymentStatistics>> {
    const params = new HttpParams({ fromObject: queryParams });
    return this.http.get<IHttpResponse<IPaymentStatistics>>(`${this.apiUrl}/${env.api.paymentsDurationStatistics}`, {
      params,
    });
  }

  getPaymentCount(queryParams: Params): Observable<IHttpResponse<IPaymentCount>> {
    const params = new HttpParams({ fromObject: queryParams });
    return this.http.get<IHttpResponse<IPaymentCount>>(`${this.apiUrl}/${env.api.paymentsByCount}`, { params });
  }

  getPaymentAmount(queryParams: Params): Observable<IHttpResponse<IPaymentCount>> {
    const params = new HttpParams({ fromObject: queryParams });
    return this.http.get<IHttpResponse<IPaymentCount>>(`${this.apiUrl}/${env.api.paymentsByAmount}`, { params });
  }
